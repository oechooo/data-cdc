...
It's possible to require items to be popped in the sequence they were pushed in instead, and in this case I would use an array instead of a min-heap to store my items. Pushes could be done faster in O(1) time, but pops would require O(n) time to enumerate the array (arranged in push order) looking for the first item with time of completion < current time.